...
def install_launcher_deps() -> None:
    subprocess.run(
        [sys.executable, "-m", "pip", "install", "-q", "openai>=1.0.0", "requests", "ddgs", "h2"],
        check=True,
    )
...
MODEL_MAIN = get_cfg("OUROBOROS_MODEL", default="openai/gpt-4o-mini", allow_legacy_secret=True)
MODEL_CODE = get_cfg("OUROBOROS_MODEL_CODE", default="anthropic/claude-sonnet-4-5", allow_legacy_secret=True)
MODEL_LIGHT = get_cfg("OUROBOROS_MODEL_LIGHT", default="openai/gpt-4o-mini", allow_legacy_secret=True)
...
os.environ["OUROBOROS_MODEL"] = str(MODEL_MAIN or "openai/gpt-4o-mini")
